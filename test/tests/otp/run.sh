...
docker run -v $OVPN_DATA:/etc/openvpn --rm -e "DEBUG=$DEBUG" $IMG ovpn_getclient $CLIENT | sed 's/auth-user-pass/auth-user-pass \/client\/credentials.txt/' | tee $CLIENT_DIR/config.ovpn
