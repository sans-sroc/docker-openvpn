...
# nopass is insecure
docker run -v $OVPN_DATA:/etc/openvpn --rm -t -e "EASYRSA_BATCH=1" -e "EASYRSA_REQ_CN=Docker OpenVPN Test CA" $IMG ovpn_initpki nopass

docker run -v $OVPN_DATA:/etc/openvpn --rm -t $IMG easyrsa build-client-full $CLIENT nopass
